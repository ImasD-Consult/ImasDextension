    ...
    <div class="flex flex-col h-full min-h-0 gap-2 text-gray-900" data-wbs-root>
      <div class="flex flex-wrap items-end gap-2 border-b border-gray-200 pb-2 shrink-0">
        <div class="flex flex-col min-w-0">
          <h2 class="text-base font-semibold leading-tight">WBS (v 6.1)</h2>
  ...
  `;
    ...
    <div class="rounded-lg border border-gray-200 p-3">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 class="text-lg font-semibold">WBS (v 6.1)</h2>
  ...
  `;
	...
	async function syncViewerSelectionFromKnownLink(link: string): Promise<void> {
		...
		if (!modelId || runtimeId == null || Number.isNaN(runtimeId)) {
			setStatus(
				`Known link selected, but could not map it to runtime selection in current model: ${trimmed}`,
				"error",
			);
			return;
		}
	...
	assignButtonEl.addEventListener("click", async () => {
		...
		const targetSummary = summarizeWriteTargets(
			selectedPartsForWrite,
			fallbackKnownLink,
		);
		...
		try {
			...
			if (apiVerification?.matchedValue) {
				...
				setStatus(
					`Assigned WBS row ${assignedRowIndex + 4} to ${selectedPartsForWrite.length} part(s). Could not verify object payload after write. PSet: def=${writeResult.defId}, prop=${writeResult.propertyName}. Targets: ${targetSummary}`,
				);
			}
		} catch (error) {
			const message =
